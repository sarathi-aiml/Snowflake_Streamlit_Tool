...
def execute_query(query):
    try:
        ...
        if 'Authentication token has expired' in str(e):
            st.error("Authentication token has expired. Reconnecting...")
            con.close()  # Close the existing connection
            con = init_connection()  # Reinitialize the connection
            cur = con.cursor()  # Recreate the cursor
            cur.execute(query)  # Retry the query
        else:
